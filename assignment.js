let domReady = (function() {
  ...
  const manipulateTemp = {
    ...
    cloneSecondary: function(cloneSec, animal) {
      ...
      cloneSec.querySelector(domAnimal.imageA).style.backgroundImage =
        'url("' + animal.image1 + '")';
      cloneSec.querySelector(domAnimal.imageA).dataset.img2 = animal.image2;
      let description = animal.description;
      cloneSec.querySelector(domAnimal.paragraphA).textContent =
        description.substring(0, 160) + "...";
      cloneSec.querySelector(
        domAnimal.paragraphAC
      ).textContent = description.substring(160, description.length);
      return cloneSec;
    }
  };
...
if (domReady == true) {
  ...
  assignEvents(
    [".animal_paragraph", ".animal_left-arrow", ".animal_right-arrow"],
    [seeMore, moveLeft, moveRight]
  );
  ...
  function seeMore() {
    if (event.target.dataset.trigger === "false") {
      ...
      let text2 = event.target.parentElement.querySelector(
        ".animal_paragraph-continuation"
      ).textContent;
  ...
  function moveLeft() {
    let image = event.target.parentElement.querySelector(
      ".animal_image-container"
    );
    image.style.backgroundImage = 'url("' + image.dataset.img1 + '")'; // you can write single quotation inside double quotations, no need to escape double quotations
  }
